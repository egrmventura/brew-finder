...
4. **Add any BJCP content**: a `bjcp_*` column, a BJCP crosswalk, or BJCP-derived vital statistics (ADR-0002)
...
Never fabricate a threshold, a version, a TTB class/type, a keyword-to-facet mapping, a barrelage figure, or a date. If a value is needed and not available, stop and ask.
...
**BJCP content is not used.** Styles come from TTB class/type plus the keyword map (`pipeline/dbt/seeds/style_keyword_map.csv`). No BJCP code, column, crosswalk, or vital statistic exists in any table (ADR-0002).